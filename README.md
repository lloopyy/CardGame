...
## 1. 需求
1. 需求1：手牌区翻牌替换
- 点击手牌区heartA，heartA会平移（简单MoveTo）到手牌区的顶部牌（club4）并替换它作为新的顶部牌
2. 需求2：桌面牌和手牌区顶部牌匹配
- 点击桌面牌的diamond3，卡牌会和手牌区顶部的club4进行匹配【桌面牌区的牌只要和手牌区顶部牌点数差1就可以匹配，无花色要求】，点击的桌面牌（diamond3）会平移到手牌区的顶部牌（club4）并替换它作为新的手牌区的顶部牌
3. 需求3：回退功能
场景：点击diamond3 -> 点击heartA -> 点击spade2 后；连续多次点击 回退按钮 ，各卡牌需要反着平移（简单MoveTo）到原位置；直到无回退记录可回退；

## 2. 开发环境及要求
1. 开发环境：cocos2dx 3.17
2. 要求：
  - 代码结构请借鉴并尽可能遵循【程序设计架构要求】；达到代码结构可维护性和可扩展性。
